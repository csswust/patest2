  	  ...
  	  var program={
			...
			getExamInfo:function(){
				$.ajax({
	 					...
	 					success:function(result){
	 						...
	 						program.starttime = nav.transTime(result.data[0].startTime);
							program.endtime = nav.transTime(result.data[0].endTime);
			...
			getProById:function(){
				 $.ajax({
	     					type : "get",
	     					content : "application/x-www-form-urlencoded;charset=UTF-8",
	     					url:"../problem/selectProblemByPrproId",		
	     					dataType : 'json',
	     					async : false,
	     					data:{
	     						"prproId" : program.papbId
	     					},
	     					success:function(result){ 
	     						console.log(result);
	     						$(".quesName").text(result.problemInfo.title);
	     						$(".time").text(result.problemInfo.limitTime+"S   ");
	     						$(".memory").text(result.problemInfo.limitMemory+"K");
			...
			submit:function(){
				 $.ajax({
	     					...
	     					data:{
	     						...
	     						"submitInfo.papbId" : program.papbId,
