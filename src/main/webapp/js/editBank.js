	 ...
	 var program ={
		...
		showproinfo:function(){
			...
			for(var i = 0;i<length;i++){	
				var level;
				if(program.levelArr[i] == 0){
					level = '容易';
				}else if(program.levelArr[i] == 1){
					level = '中等';
				}else if(program.levelArr[i] == 2){
					level = '困难';
				}
				program.html += '<tr class="'+flag+'-'+flag+'">'
						...
						+'<td>' + level + '</td>'
						+'<td>' + program.couseNamesArr[i] + '</td>'
						+'<td>' + program.knowNameArr[i] + '</td>'
		...
		selectByExamId:function(){
			...
			$.ajax({
				...
				success:function(result){
					...
					program.knowNameArr = result.knowNameList;
					program.couseNamesArr = result.couseNames;
