	...
	var program = {
			...
			$.ajax({
					...
					success:function(result){ 
						...
						if(result.data[0].isCourse == 1){
							...
							/*$(".choose option[value="+ result.data[0].isCourse +"]").attr("selected",true);*/
							$(".choose").val(0);
							$(".courseName option[value="+ result.data[0].parentId +"]").attr("selected",true);
							$(".knowName").val(result.data[0].knowName);
						}
					},
					error:function(){
						console.log("error");
		...
		getCourseInfo:function(){
			$.ajax({
					...
					success:function( result ) {
                        ...
                        program.courseName = result.courseList;
                        program.sumList = result.sumList;
                        program.course = result.data;
                        for (var i = 0; i < result.data.length; i++) {
                        	if (result.data[i].isCourse == 0) {
                                if (result.courseList[i].knowName == 'C程序设计') {
                                    $("#ClistInfo").append('<li class="list-group-item">' +
                                        ...
                                        '</li>');
                                } else if (result.courseList[i].knowName == '数据结构') {
                        ...
                        $("#ClistInfo li").each(function (i) {
                            $("span:last a.title", this).click(function () {
		...
		selectOnlyCourse:function(){
			$.ajax({
				...
				success:function( result ){
					console.log(result);
					program.courseName = result.data;
				}
			});
		},
		addKnow:function(){
