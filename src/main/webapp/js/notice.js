	...
	var program = {
		...
		addNotice:function(){
			$.ajax({
				...
				url : "../examNotice/insertExamNotice",
				dataType : 'json',
				async : false,
				data : {
		...
		showNotice:function(){
			...
			for(var i = 0;i < length;i++){
				program.html+=
					'<tr><td><input type="checkbox" value="'+program.data[i].stnoId+'" name="title"/></td>'
				+ '<td>'+program.data[i].stnoId+'</td>'	
		...
		updateNotice:function(){
			...
			$.ajax({
				type : "get",
				content : "application/x-www-form-urlencoded;charset=UTF-8",
				url : "../examNotice/updateExamNotice",
				dataType : 'json',
				async : false,
				data : {
					stnoId:program.stnoId,
					content:program.describle,
					title:program.title,
				},
				success : function(result) {
