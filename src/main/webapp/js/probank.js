	...
	var program = {
		...
		showProblem : function() {
			...
			for (var i = 0; i < length; i++) {
				var difficult = "";	var courseName = "";var knowName = "";
 				if(program.data.data[i].level == 0){
 						difficult = "容易";
 				}else if(program.data.data[i].level == 1){
 						difficult = "中等";
 				}else if(program.data.data[i].level == 2){
 					    difficult = "困难";
 				}
				...
				var str = '<tr>' + '<td><input type="checkbox" value="'
					...
					str += ' name="title" class="show"/></td>' + '<td>'
						...
						+ '<td>' + difficult + '</td>' + '<td>'
						+ program.data.data[i].acedNum + '/'
						+ program.data.data[i].totalSubmit + '</td>'
